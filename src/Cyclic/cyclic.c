/*
 * LU : Cyclic Decomposition with MTE and PE data Transfer ...
 * cyclic/cyclic.c
 *
 * This program parallelizes a LU algorithm using Cyclic Decomposition of the Matrix
 * to get its LU.
 *
...
#define LOCAL_MAT_ROWS  8             // Local matrix's number of Rows ...                                      
...
/*
 * submatrixCalculations(int pivotRow, int myMin)
 *
 * This Function calculates the submatrix of a block of cyclic rows
 *
*/

void submatrixCalculations(int pivotRow, int myMin)
 {
     int i = 0; // Row Wise Index 
     int j; // Column Wise Index ...
	
	 for(i=myMin ; i<LOCAL_MAT_ROWS;i+=PES)  // i=i+PES for row cyclic add PES to get next value  
	  for(j=pivotRow+1;j<SIZE;j++)
	    local_matA[i][j]=local_matA[i][j]-local_matA[i][pivotRow]*pivot_row[j];
			
 }

int myMinCalculations(int pivotRow, int isPivotalBlock, int my_peid)
 {
	 int myMin = my_peid;
     int nextNPECycles;
     
	 if(isPivotalBlock!=1)
		 {		// its not the pivotal block 
			myMin=my_peid;			
		 }
		else
		 {						//its the pivotal block
		   for (nextNPECycles=((pivotRow+1)%LOCAL_MAT_ROWS);nextNPECycles <(((pivotRow+1)%LOCAL_MAT_ROWS)+PES);nextNPECycles++)
		    {
		 	  if(my_peid == (nextNPECycles%PES))
			     myMin=nextNPECycles;
			}
			
			if((pivotRow+1)%LOCAL_MAT_ROWS == 0) myMin = LOCAL_MAT_ROWS;
	  	  }// end of if-else
...
void calculate()
{
  ...
  int my_min = my_peid;				// starting row of the process
  int blockCount = 0;
  int isPivotalBlock ;
  ...
  for(k=0;k<SIZE;k++)
  {
	...
	for (blockCount = (k/LOCAL_MAT_ROWS); blockCount < (SIZE/LOCAL_MAT_ROWS); blockCount++)
	 {
		if(my_peid==0) // Giving PE0 the responsibility to get data from SDRAM
    	   moveDataToLocalMemory(blockCount);
	    
       barrier(k, my_peid); // ******* SYNCHRONIZE HERE -- 1
           
           my_min = myMinCalculations(k, isPivotalBlock, my_peid);
            
 	       if(((k % PES) == my_peid)&&(isPivotalBlock == 1)) 
 	        {
	 	      initializePivotRow();
	 	      pivotalRowCalculations(k);
	        } //End of Pivot Row Calc

	       // my_min = myMinCalculations(k, isPivotalBlock, my_peid);
   
	   barrier(k, my_peid);	// ******* SYNCHRONIZE HERE -- 2   	
	   
	       isPivotalBlock = 0;
	
           submatrixCalculations(k, my_min);
		
	   barrier(k, my_peid);	 // ******* SYNCHRONIZE HERE -- 3
		  	
	       if(my_peid==0)// get data from SDRAM
	         moveDataToSDRAM(blockCount);
	   
	   barrier(k, my_peid); // ******* SYNCHRONIZE HERE -- 4
	
	} // End of blockCount Loop ...  
  } // End of for(k=0;k<SIZE;k++)
